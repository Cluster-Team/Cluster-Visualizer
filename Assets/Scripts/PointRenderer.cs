...
//modifying color scheme to be decided by label column, and creating animated frames
//frame list is modified by changing animated frames through the csv
public class PointRenderer : MonoBehaviour {
    ...
    // public int labelColumnStart=0;
    public int labelColumnOffset=3;
    public int labelColumn=3;
    public int labelFrame=34;
    ...
    void Start () 
	{
        ...
        labelIndex=((int)(Time.time)) % (labelFrame);
        labelColumn=labelIndex+labelColumnOffset;
        ...
        labelColumnName=columnList[labelColumn];
        ...
        labelMax= FindMaxValue(labelColumnName);
        ...
        labelMin=FindMinValue(labelColumnName);
            
        // Debug.Log(xMin + " " + yMin + " " + zMin); // Write to console

        AssignLabels();
        dps = new GameObject[pointList.Count];
	...
	void Update ()
    {
        int label_index_prev= labelIndex;
        labelIndex = ((int)(Time.time)) % (labelFrame);
        if(labelIndex!=label_index_prev){
            labelColumn=labelColumnOffset+labelIndex;
            labelColumnName=columnList[labelColumn];
            labelMin=FindMinValue(labelColumnName);
            labelMax= FindMaxValue(labelColumnName);
                for (var i = 0; i < pointList.Count; i++){
                float normalised_label=(Convert.ToSingle(pointList[i][labelColumnName]) - labelMin)/ (labelMax - labelMin);
                Color color = Color.HSVToRGB(normalised_label * 0.7f, 1f, 1f);
                // Color color = Color.HSVToRGB(0,0,0);

                dps[i].GetComponent<Renderer>().material.color = color;

                // Activate emission color keyword so we can modify emission color
                dps[i].GetComponent<Renderer>().material.EnableKeyword("_EMISSION");

                dps[i].GetComponent<Renderer>().material.SetColor("_EmissionColor", color);
                }
            // for (int i = 0; i < particlePoints.Length; i++)
            // {
                // GetComponent<ParticleSystem>().Emit(particlePoints[i], 1);
                // particlePoints[i].remainingLifetime = -1f;
                // GetComponent<ParticleSystem>().Emit(particlePoints[i], 1);
            // }
            // Start();
        }
	...
	private void PlacePrefabPoints()
	{
                ...
                for (var i = 0; i < pointList.Count; i++)
        {
			...
			dps[i] = Instantiate (PointPrefab, Vector3.zero, Quaternion.identity);

            
            // Make child of PointHolder object, to keep particlePoints within container in hiearchy
            dps[i].transform.parent = PointHolder.transform;

            // Position point at relative to parent
            dps[i].transform.localPosition = position;

            dps[i].transform.localScale = new Vector3(pointScale, pointScale, pointScale);

            // Converts index to string to name the point the index number
            string dataPointName = i.ToString();

            // Assigns name to the prefab
            dps[i].transform.name = dataPointName;
    ...
    private void CreateParticles()
    {
       ...
       // Debug.Log("Row Count is " + rowCount);
        ...
        for (int i = 0; i < pointList.Count; i++)
        {
            ...
            // Set point location
            particlePoints[i].remainingLifetime = 100f;

			particlePoints[i].position = new Vector3(x, y, z) * plotScale;
          
            //GlowColor = 
            // Set point color
            particlePoints[i].startColor = new Color(0, 0, 0, 1.0f);
